...
sidecar and cni-sidecar topologies; `full` does not restore binary-scoped
network identity.
...
is deleted. The agent process supervisor also defaults to `network-only`; set
`process_enforcement = "full"` only when you want combined-mode
process/filesystem guards in the agent container.
